...
ALLOWED_LUT_EXTENSIONS = {".cube", ".3dl", ".lut"}
...
def sanitize_error(err: Exception) -> str:
    ...
    msg = re.sub(r"api[_-]?key\s*[:=]\s*\S+", "api_key=[REDACTED]", msg, flags=re.IGNORECASE)
    msg = re.sub(r"\b\d{1,3}(?:\.\d{1,3}){3}\b", "[IP]", msg)
    return msg[:300]


def validate_path(p: str, label: str, must_exist: bool = True) -> Path:
    """Validate a filesystem path."""
    if not p or len(p) > MAX_PATH_LENGTH:
        raise ValueError(f"Invalid {label}: empty or exceeds {MAX_PATH_LENGTH} chars")
    resolved = Path(p).resolve()
    if must_exist and not resolved.exists():
        raise FileNotFoundError(f"{label} not found: {resolved.name}")
    return resolved


def validate_video_input(path: Path) -> Path:
    """Validate video file extension and size."""
    if path.suffix.lower() not in ALLOWED_VIDEO_EXTENSIONS:
        raise ValueError(f"Unsupported video type: {path.suffix}. Allowed: {', '.join(sorted(ALLOWED_VIDEO_EXTENSIONS))}")
    if path.stat().st_size > MAX_FILE_SIZE_BYTES:
        raise ValueError(f"File too large (max {MAX_FILE_SIZE_BYTES // (1024**3)} GB)")
    if path.stat().st_size == 0:
        raise ValueError("File is empty")
    return path


def validate_lut_path(lut: Optional[str]) -> Optional[str]:
    """Validate LUT file path if provided."""
    if not lut:
        return None
    lut_path = validate_path(lut, "LUT file")
    if lut_path.suffix.lower() not in ALLOWED_LUT_EXTENSIONS:
        raise ValueError(f"Invalid LUT format. Allowed: {', '.join(sorted(ALLOWED_LUT_EXTENSIONS))}")
    return str(lut_path)
